  ...
  return (
    <div className="min-h-screen bg-slate-100">
      <aside
        ...
        className={`fixed inset-y-0 left-0 z-40 overflow-hidden bg-[#111c2f] shadow-2xl transition-[width] duration-300 ease-in-out ${
      ...
      >
        <div className="flex h-full w-[304px] flex-col">
          ...
          <nav className="flex-1 px-3 py-6">
            <p
              className={`mb-3 px-3 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 transition-opacity duration-200 ${
                isSidebarExpanded ? "opacity-100" : "opacity-0"
              }`}
            >
              Opérations
            ...
            {TABS.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`group mb-1 flex h-12 w-full items-center border-l-2 px-3 text-left transition-colors ${
                    isActive
                      ? "border-[#f28c28] bg-white/10 text-white"
                      : "border-transparent text-slate-300 hover:bg-white/5 hover:text-white"
                  }`}
                >
                  <Icon className="h-5 w-5 shrink-0" aria-hidden="true" />
                  <span
                    className={`ml-4 whitespace-nowrap text-sm font-semibold transition-opacity duration-200 ${
                      isSidebarExpanded ? "opacity-100" : "opacity-0"
                    }`}
                  >
                    {item.label}
          ...
          <div className="border-t border-white/10 px-6 py-5">
            <p
              className={`whitespace-nowrap text-xs text-slate-400 transition-opacity duration-200 ${
                isSidebarExpanded ? "opacity-100" : "opacity-0"
              }`}
            >
              Gestion obligataire
      ...
      <main
        ...
        style={{ paddingLeft: isSidebarExpanded ? "304px" : "76px" }}
      >
        <header className="border-b border-slate-200 bg-white px-8 py-7">
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-[#0f1f3d]">Espace Back Office</h1>
              <p className="mt-1 text-sm text-slate-500">Connecté en tant que {email}</p>
            </div>
            <p className="border-l-2 border-[#f28c28] pl-3 text-sm font-semibold text-slate-600">{activeTab?.label}</p>
          </div>
        </header>
